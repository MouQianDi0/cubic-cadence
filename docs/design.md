...
阶段 2 采用合法的短 WAV 文件和整段 PCM 静态缓冲，只用于隔离验证解码、输出和生命周期。音乐界面直接复用原版 HUD 的 `experience_bar_background` 与 `experience_bar_progress` sprite，将条体加长到与音量控件一致的 220 像素，以灰色槽和绿色填充展示进度，并叠加带深色描边的像素拖动点；纯时间数值显示在经验条正下方居中。播放/暂停切换和停止使用紧凑媒体图标按钮，并通过 Tooltip 保留当前操作的本地化文字说明。播放中或暂停中拖动时只更新条体、拖动点和目标时间预览，不连续修改声音位置；释放鼠标后才通过当前静态 OpenAL 声源的 `AL_SEC_OFFSET` 执行一次 seek，并同步内部播放时钟，避免连续重定位产生杂音。在线长音频仍必须在后续阶段改为有限大小的流式缓冲队列，并基于流式解码器单独实现 seek，不能直接沿用静态缓冲方案。
